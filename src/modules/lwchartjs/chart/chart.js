...
import { parseBoolean, sanitize } from 'lwchartjs/utils';
import { nanoid } from 'lwchartjs/nanoid';
...
import {
    ...
    ERROR_EVENT_NAME,
} from 'lwchartjs/constants';
import ChartConfigService from 'lwchartjs/chartConfigService';
import ReactivityManager from 'lwchartjs/reactivityManager';
...
export default class Chart extends LightningElement {
    ...
    drawChart() {
        ...
        try {
            ...
            if (!this._chart || !this._chart.ctx) {
                ...
                console.log('CONFIG BEFORE CHART DRAW:');
                console.log(JSON.stringify(config, null, 2));
                this._chart = new chartjs.Chart(this.getCanvas().getContext('2d'), {
                    type: this._type,
                    data: this._details,
                    options: config,
                });
                // const ctx = this.template.querySelector('div.chart').getContext('2d');
    ...
    _isReadyToDraw() {
        return this._chartjsLoaded && this._details && this._type /* && this.isConnected*/;
    }
    ...
    _listenerHandlers = {
        ...
        handleOption: (evt) => {
            ...
            if (option === ATTRIBUTE_DATA) {
                ...
                console.log('Calling updateConfig from handleOption in the chart parent cmp');
