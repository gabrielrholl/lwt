...
import { POLARAREA_CHART_TYPE, RADAR_CHART_TYPE, PIE_CHART_TYPE, DOUGHNUT_CHART_TYPE } from 'lwchartjs/constants';
        ...
        try {
            ...
            Promise.resolve()
                .then(() => {
                    ...
                    this.dimensionsLabels = this.dimensionsLabels || [...new Set(data.map((x) => x.labels).flat())];
                    this._details = data
                        .filter((x) => !!x.detail)
                        .map((x, i) => ({
                            detail: x.detail,
    ...
    set soql(v) {
        ...
        if (this._soql) {
            ...
            this._soql = this._soql.replace(
                /:recordId/g,
                `'${this.recordId ? this.recordId.replace("'", "\\'") : ChartBuilder.FAKE_ID}'`,
            );
            // pass the SOQL to the getData service from the server
            this._getChartDataHandler(ChartBuilder.SOQL_DATA_PROVIDER_APEX_TYPE, this._soql);
        }
    }
